...
class Options():
    def __init__(self):
        ...
        self.parser.add_argument('--name', type=str, default='cam4', help='Specify cam4 or cam5 as cam4 is the front camera while cam5 is the back camera')
        ...
        self.parser.add_argument('--percent', type=float, default=97.0, help='for estimating threshold based on valid positive samples')
        self.parser.add_argument('--bg_mask', type=str, default=None, help='background mask, B means black, W means white')
        self.parser.add_argument('--filter', type=int, default=9, help='size of median filter')
        self.parser.add_argument('--edge_filter', type=int, default=13, help='size of median filter')
        self.parser.add_argument('--mor_filter', type=int, default=4, help='size of median filter')
        self.parser.add_argument('--ssim_win', type=int, default=7, help='size of median filter')


    def parse(self):
        DATASET_PATH = '../../../../../Datasets/Paper'
        ...
        if not self.opt.save_dir:
            self.opt.save_dir = './results/'+self.opt.name+'/reconst/'+self.opt.loss
